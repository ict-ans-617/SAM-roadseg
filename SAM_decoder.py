...
class ConvModule(nn.Module):
    def __init__(self, in_ch=256, out_ch=256, kernel_size=0, padding=0, stride=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=kernel_size, stride=stride, padding=padding),#in_ch = 1024 ,out_ch = 256
...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, inputs):
        image_embedding, inner_states = inputs          #inner_staes为image_encoder中ImageEncoderViT返回的out
        ######################
        # print("Shapes of inner_states:")
        # for idx, tensor in enumerate(inner_states):     
...
class SegHead(nn.Module):
    def __init__(self, model_size):
        ...
        if model_size == "h":
            ...
            self.neck_net = SAMAggregatorNeck(in_channels=[768]*12, selected_channels = range(4, 12, 2))
